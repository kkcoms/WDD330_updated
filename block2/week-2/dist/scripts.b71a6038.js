...
// Chapter 8
// Animation example from chapter 9
var squareElement = document.getElementById('square');
var angle = 0;
setInterval(function () {
  angle = (angle + 2) % 360;
  squareElement.style.transform = "rotate(".concat(angle, "deg)");
}, 1000 / 60); //Request Animation Frame from chapter 9

var squareElement1 = document.getElementById('square1');

function rotate() {
  angle = (angle + 2) % 360;
  squareElement1.style.transform = "rotate(".concat(angle, "deg)");
  window.requestAnimationFrame(rotate);
}

var id = requestAnimationFrame(rotate); //Quiz Ninja from chapter 9 and 14

var url = 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/123941/questions.json';
fetch(url).then(function (res) {
  return res.json();
}).then(function (quiz) {
  view.start.addEventListener('click', function () {
    return game.start(quiz.questions);
  }, false);
